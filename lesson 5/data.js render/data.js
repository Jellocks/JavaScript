...
// Разделение на три блока: tv shows, movies, cartoons
const tvShows = data.filter((item) => item.type === 'show');
const movies = data.filter((item) => item.type === 'movie');
const cartoons = data.filter((item) => item.type === 'cartoon');

// Отрисовка данных из каждого блока
    ...
    const itemHtml =
      `<div class="item">
        <img src="${item.image}" alt="${item.title}">
          ...
          <p>Director: ${item.director.join(', ')}</p>
          <p>Writers: ${item.writers.join(', ')}</p>
          <p>IMDB ID: ${item.imdbid}</p>
      </div>`;
    itemsWrapper.innerHTML += itemHtml;
  });
  container.appendChild(itemsWrapper);
  document.body.appendChild(container);
}

renderData(tvShows, 'TV Shows');
renderData(movies, 'Movies');
renderData(cartoons, 'Cartoons');